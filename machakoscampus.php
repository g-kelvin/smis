      ...
      <section class="wrapper">
        <div class="row">
          ...
          <?php 
                            ...
                            $feedback="SELECT * from students WHERE campus = 'Machakos' AND status = 'active' ORDER BY studentid DESC  LIMIT 10 ";
                            if ($result=mysqli_query($con,$feedback))
                            {
                            // Return the number of rows in result set
                            $rows=mysqli_num_rows($result);
                            ...
                            $feedback="SELECT * from students WHERE campus = 'Machakos' AND status = 'Graduates' ";
                            if ($result=mysqli_query($con,$feedback))
                            {
                            // Return the number of rows in result set
                            $rows=mysqli_num_rows($result);
                            echo "<b style = 'color: red'> Active Students are = ". $rows. "</b>";
                            // Free result set
                            mysqli_free_result($result);
                            }
...
<div class="row">
  ...
  <form action="" method="post">
    <div class="col-sm-6 form-group">
                ...
                <option selected>Select the Campus</option>
                <option value="Machakos">Nairobi</option>
  ...
  <div class="col-sm-7">
    ...
    <?php
                                ...
                                $qry=mysqli_query( $con," INSERT INTO `users` ( `message`, `user_name`, `pass`, `status`, `message_from`) VALUES ('$message', '".$rows['user_name']."', '".$rows['pass']."', 'Unread', '$message_from')" );
                                  if ($qry){
                               ...
                               $qrt='SELECT * from users WHERE message_from ="Kitui" OR message_from ="Nairobi"  and status = "Unread" ORDER BY id DESC  LIMIT 3';
